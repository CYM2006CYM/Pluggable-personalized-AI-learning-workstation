...
> 用途：说明多智能体内容生成、审核、返修与发布机制，并为项目展示、案例记录和测试复核提供统一口径。
>
> Hunter负责提出问题，不拥有路由和裁决权。只有 Hunter 报告至少一项 `severity=high` 问题时才触发 Defender；Judge 综合正文、候选、Hunter 问题和必要的 Defender 辩护独立裁决，可以推翻不成立的指控。可修复问题经 Judge 确认后返回 Generator 完整重生成。
...
系统实现“Judge授权返修 → Generator → 安检 → Hunter → 必要时Defender → Judge”的闭环。角色权限、high-only 路由、逐项证据合同、Judge 独立裁决、合同失败与正式拒绝区分、accepted 证明绑定、Safety 审计、超时所有权和公开时间线均遵循本文规则。
