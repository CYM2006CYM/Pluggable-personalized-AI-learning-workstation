...
describe("W5 D3 C real public execution package measurement", () => {
  it("consumes ten real /run packages and measures each with Node three times", async () => {
    ...
    try {
      ...
      for (let step = 0; step < 40 && seen.size < 5; step += 1) {
        ...
        if (opened.body.data.kind !== "code") {
          ...
          let finalCode = opened.body.data.userText;
          ...
          const submitted = await post(server.url, `/api/activities/${activity.activityId}/submit`, { ...writeMeta(draftState, `r2-submit-${step}`), kind: "code", activityId: activity.activityId, activityVersion: opened.body.data.activity.activityVersion, attemptId: opened.body.data.attemptId, userText: finalCode, draftVersion: draftState.draftVersion });
    ...
    expect(new Set(groups.map((group) => group.activityId)).size).toBe(5);
