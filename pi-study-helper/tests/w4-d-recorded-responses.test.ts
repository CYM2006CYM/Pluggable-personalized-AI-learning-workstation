...
describe("W4 D recorded response set", () => {
  ...
  it("keeps historical candidates frozen and rejects them after the authoritative lesson digest changes", async () => {
    ...
    await expect(service.prepareCard({ profileRevision: 3, knowledgePointId: "pandas.clean.read-csv", excludedArtifactIds: ["card-w4-read-csv"] }))
      .resolves.toEqual({ status: "unavailable" });
    expect(adapter.history.length).toBeGreaterThanOrEqual(2);
    expect(adapter.history.every((item) => item.input.graphId === "generator")).toBe(true);
  });
