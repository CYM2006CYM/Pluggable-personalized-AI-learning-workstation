...
async function setupFormalScenario(suffix: string, result: ActivityResult) {
  ...
  const pathCandidate = { pathId: `path-${suffix}`, pathVersion: 1, status: "candidate" as const, goalId: "goal-1", mode: "recommended" as const, nodes: [{ nodeId: `node-${suffix}`, knowledgePointId: "kp-dataframe", activityIds: [assignment.activityId], status: "available" as const, estimatedMinutes: 5, reasonCodes: ["goal_required"], difficulty: "S-U" as const, scaffold: "none" as const, required: true, positionLocked: false }] };
