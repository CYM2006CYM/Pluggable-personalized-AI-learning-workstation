...
function validateRequest(name: keyof LearningRuntimeFacade, value: JsonObject): void {
  switch (name) {
    ...
    case "buildPath":
      exact(value, ["requestId", "sessionId", "sessionVersion", "profileRevision", "goalId", "mode", "availableMinutes", "evidenceVersion", "selectedKnowledgePointIds", "lockedNodeIds"], ["chapterId"]);
      writeMeta({ requestId: value.requestId, sessionId: value.sessionId, sessionVersion: value.sessionVersion, profileRevision: value.profileRevision }); stringField(value.goalId, "goalId", true); enumField(value.mode, "mode", ["recommended", "chapter"]); integerField(value.availableMinutes, "availableMinutes"); integerField(value.evidenceVersion, "evidenceVersion"); stringArray(value.selectedKnowledgePointIds, "selectedKnowledgePointIds"); stringArray(value.lockedNodeIds, "lockedNodeIds"); if (value.chapterId !== undefined) stringField(value.chapterId, "chapterId", true); if (value.mode === "chapter" && value.chapterId === undefined) throw invalidSemantic("Chapter mode requires chapterId."); if (value.mode === "recommended" && value.chapterId !== undefined) throw invalidSemantic("Recommended mode cannot include chapterId."); return;
    case "confirmPath":
