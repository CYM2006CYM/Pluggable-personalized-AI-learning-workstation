...
export function ActivityPage() {
  ...
  const persistCodeDraft = async (current: CodeActivityDraftOutput): Promise<CodeActivityDraftOutput> => {
    const saved = await api.saveActivityDraft({
      ...
      attemptId: current.attemptId, draftVersion: current.draftVersion, userText: localDraft,
    });
    if (saved.kind !== "code") throw new Error("invalid_code_draft_response");
    setOpened(saved);
    return saved;
  };
  ...
  const preview = async () => {
    ...
    setBusy(true); setActionError(undefined);
    try {
      const saved = await persistCodeDraft(opened);
      const prepared = await api.prepareActivityRun({ requestId: newRequestId("web-preview"), sessionId, sessionVersion: saved.sessionVersion, profileRevision: saved.profileRevision, activityId, activityVersion: saved.activity.activityVersion, attemptId: saved.attemptId, draftVersion: saved.draftVersion, mode: "preview" });
      setPreparedText(`${prepared.environmentId} · ${prepared.publicTestSources.length} 项公开检查`);
    } catch (error) { await captureActionError(error, "preview_failed"); }
    finally { setBusy(false); }
  };
  ...
  const retryEvaluation = async () => {
    ...
    try {
      ...
      if (fresh.session === undefined || attempt?.kind !== "code" || attempt.attemptId !== opened.attemptId || attempt.status !== "evaluator_error") {
        throw new Error("evaluator_attempt_not_recoverable");
      }
      const recovered = await api.recoverActivity({
        sessionId, sessionVersion: fresh.session.view.sessionVersion, profileRevision: fresh.session.view.profileRevision,
        activityId, attemptId: attempt.attemptId,
      });
      if (recovered.attempt.kind !== "code" || recovered.draftVersion === undefined || recovered.userText === undefined) {
        throw new Error("evaluator_draft_not_recoverable");
      }
      const recoveredDraft: CodeActivityDraftOutput = {
        ...opened,
        sessionVersion: recovered.sessionVersion,
        profileRevision: recovered.profileRevision,
        draftVersion: recovered.draftVersion,
        userText: recovered.userText,
      };
      setDraft(recoveredDraft.attemptId, recoveredDraft.userText);
      setOpened(recoveredDraft);
      const retried = await api.submitActivity({
        requestId: newRequestId("web-retry-evaluation"), sessionId,
        sessionVersion: recoveredDraft.sessionVersion, profileRevision: recoveredDraft.profileRevision,
