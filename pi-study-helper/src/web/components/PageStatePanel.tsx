...
import { statePanelCodeLabel } from "../copy/ui-copy.js";
...
/*
 * 面板左上的小字位不再打印原始错误码——那是内部标识符。
 * 统一走 statePanelCodeLabel 翻成中文大类;原始码进 data-error-code,
 * 开发者工具里仍可查,界面上使用者只看到能读懂的大类。
 */
export function PageStatePanel({ page, state, code, detail, onRetry }: PageStatePanelProps) {
  const pageLabel = pageLabels[page];
  const codeLabel = statePanelCodeLabel(code);

  if (state === "loading") {
    return <section className="state-panel" data-state="loading" aria-busy="true" aria-live="polite" style={{ minHeight: STABLE_LAYOUT.statePanelMinHeight }}><p className="state-code">加载中</p><h2>正在加载{pageLabel}</h2><p>正在从本地服务读取安全快照。</p></section>;
  }

  if (state === "empty") {
    return (
      <section className="state-panel" data-state="empty" aria-live="polite" style={{ minHeight: STABLE_LAYOUT.statePanelMinHeight }}>
        <p className="state-code">暂无内容</p>
        <h2>暂无{pageLabel}</h2>
  ...
  if (state === "error") {
    return (
      <section className="state-panel error-state" data-state="error" data-error-code={code} role="alert" style={{ minHeight: STABLE_LAYOUT.statePanelMinHeight }}>
        <p className="state-code">{codeLabel}</p>
  ...
  return (
    <section className="state-panel recovery-state" data-state="recovery" data-error-code={code} aria-live="polite" style={{ minHeight: STABLE_LAYOUT.statePanelMinHeight }}>
      <p className="state-code">{codeLabel}</p>
