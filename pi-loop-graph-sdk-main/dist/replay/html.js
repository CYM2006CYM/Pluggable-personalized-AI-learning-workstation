export function exportReplayHtml(model) {
...
function renderContext(ctx) {
    ...
    <div class="ctx-blocks">${ctx.blocks.map(b => `<pre class="ctx-block">${escapeHtml(b.text)}</pre>`).join("")}</div>
  </details>`;
}
function renderTurn(turn, completions) {
...
function renderToolCall(tc, completions) {
...
function renderCompletionVerdict(completion) {
    if (!completion)
        return "";
...
function renderEvents(title, events) {
    if (events.length === 0)
        return "";
    return `<details><summary>${escapeHtml(title)} (${events.length})</summary><ol class="raw-events">${events.map(ev => `<li><b>${escapeHtml(ev.event.type)}</b> <small>${escapeHtml(ev.timestamp)}</small><pre>${escapeHtml(JSON.stringify(ev.event.data ?? {}, null, 2))}</pre></li>`).join("")}</ol></details>`;
}
// ── helpers ──
function allEvents(model) {
    const collect = (invs) => invs.flatMap(inv => [...inv.events, ...collect(inv.children)]);
    return [...model.unscopedEvents, ...collect(model.invocations)].sort((a, b) => a.sequence - b.sequence);
}
function domainIcon(d) {
    const icons = { root: "🏠", graph: "📊", node: "📍", agent: "🤖", model: "💬", tool: "🔧", completion: "✅", mechanism: "⚙️", context: "📥", compaction: "🗜️", transition: "➡️", checkpoint: "💾", recording: "📝" };
    return icons[d] ?? "•";
}
function summarizeValue(value, maxLen = 60) {
    ...
    if (value && typeof value === "object") {
        ...
        if (keys.length === 0)
            return "{}";
        const kv = keys.slice(0, 3).map(k => `${k}=${summarizeValue(value[k], 20)}`).join(", ");
...
function summarizeResult(value) {
    if (value && typeof value === "object" && !Array.isArray(value)) {
        ...
        const parts = [];
        if ("decision" in obj)
            parts.push(`decision=${obj.decision}`);
        if ("reason" in obj)
            parts.push(`reason="${String(obj.reason).substring(0, 80)}"`);
        if (parts.length > 0)
            return parts.join(", ");
