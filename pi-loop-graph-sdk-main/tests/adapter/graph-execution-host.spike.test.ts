...
describe("Item 2 — 子会话配置继承", () => {
  ...
  it("两个 session 可以使用不同的 thinkingLevel", async () => {
    ...
    // thinkingLevel 各自独立（注意：无模型时可能被 clamp 到相同值）
    expect(s1.thinkingLevel).toBe("off");
    // 在高版本 pi 中 thinkingLevel 可能因模型能力 clamp；
    // 此处只验证两个 session 的设置互不影响
    expect(typeof s2.thinkingLevel).toBe("string");
...
describe("Item 4 — Abort 信号传播", () => {
  it("session.abort() 终止正在运行的 prompt", async () => {
    ...
    // 无 API key 时 prompt 会直接 reject，也在预期范围内
    try {
      await promptPromise;
    } catch {
      // API key 缺失 — 预期行为
...
describe("Item 5 — 清理完整性", () => {
  it("dispose 后 subscribe 回调不再触发", async () => {
    ...
    // 发一个简单 prompt 触发事件（无 API key 时跳过 LLM 步骤）
    try {
